...
//     if (lowerName.includes('employment') || lowerName.includes('contract')) return 'employment';
//     if (lowerName.includes('nda') || lowerName.includes('non-disclosure')) return 'nda';
//     if (lowerName.includes('lease') || lowerName.includes('rental')) return 'lease';
//     if (lowerName.includes('service') || lowerName.includes('agreement')) return 'service';
//     return 'general';
//   };
...
//       const response = await fetch('https://document-analyzer-1-backend.onrender.com/api/documents', {
//         headers: {
...
//         console.log("Document found in location state:", location.state.document);
...
//   const documentType = documentData ? getDocumentType(documentData.DocumentName || documentData.name) : 'general';
...
//           msg.text === data.content && msg.sender === 'ai'
//         );
...
//     const documentName = documentData?.DocumentName || documentData?.name || "your document";
...
//   // Fetch chat history when document data is available - IMPROVED
//   useEffect(() => {
...
//     } else {
//       console.log('Document data not yet available, waiting...');
//     }
//   }, [documentData]); // Only depend on documentData

//   // Also try to fetch chat history when user becomes available
//   useEffect(() => {
//     if (user && documentData && documentData._id && messages.length === 0) {
//       console.log('User available, fetching chat history...');
//       fetchChatHistory(documentData._id);
//     }
//   }, [user, documentData, messages.length]);
...
//   const documentSummary = documentData.summary || "No summary available for this document.";
...
//                         <span key={index} className="bg-purple-50 text-purple-700 border border-purple-200 rounded-full px-2 py-1 text-xs">
...
//                       <h3 className="font-semibold text-gray-900">Key Events</h3>
//                     </div>
//                     <div className="space-y-1">
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.success && data.chatHistory) {
        ...
        if (Array.isArray(data.chatHistory) && data.chatHistory.length > 0) {
          ...
          const formattedMessages = data.chatHistory.map((msg, index) => {
            ...
            if (typeof msg === 'string') {
              ...
              messageSender = (msg.role === 'user' || msg.sender === 'user') ? 'user' : 'ai';
        ...
        console.log('Backend returned no chat history, showing welcome message');
        setMessages([getWelcomeMessage()]);
      }
    } else {
      console.error('Failed to fetch chat history, status:', response.status);
